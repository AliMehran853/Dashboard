...
import { AlertTriangle, ShoppingBasket, Plus, X, Trash2 } from 'lucide-react';
...
function ShoppingList() {
    ...
    const sortShoppingItems = useCallback((list) => {
    ...
    const loadItems = useCallback(async () => {
        try {
            ...
            setItems(sortShoppingItems(Array.isArray(data) ? data : []));
        } catch (loadError) {
            console.error('Failed to load shopping items:', loadError);
            setError(loadError?.message || t('shoppingList.errors.load'));
        } finally {
            setLoading(false);
        }
    }, [sortShoppingItems, t]);
    ...
    const handleSubmit = async (itemData) => {
        ...
        try {
            ...
            const quantityValue = Number(itemData.quantity);

            const preparedItem = {
                name: itemData.name?.trim() || '',
                quantity: quantityValue,
                unit: itemData.unit?.trim() || t('shoppingList.defaultUnit'),
            ...
            if (!preparedItem.name) {
                throw new Error(t('shoppingList.errors.nameRequired'));
            }

            if (!Number.isFinite(preparedItem.quantity) || preparedItem.quantity <= 0) {
                throw new Error(t('shoppingList.errors.quantityInvalid'));
            }

            const allowed = ['low', 'normal', 'high', 'urgent'];
            if (!allowed.includes(preparedItem.priority)) {
                preparedItem.priority = 'normal';
            }

            if (editingItem) {
                const updated = await updateShoppingItem(editingItem.id, preparedItem);
                if (!updated) {
                    throw new Error(t('shoppingList.errors.itemNotFoundAfterEdit'));
                }
                setItems((prev) =>
                    sortShoppingItems(prev.map((it) => (it.id === updated.id ? updated : it)))
                );
            } else {
                const newItem = await addShoppingItem({ ...preparedItem, completed: false });
                setItems((prev) => sortShoppingItems([newItem, ...prev]));
            }

            setShowForm(false);
            setEditingItem(null);
        } catch (saveError) {
            console.error('Failed to save shopping item:', saveError);
            setError(saveError?.message || t('shoppingList.errors.save'));
            throw saveError;
        } finally {
            setSaving(false);
        }
    ...
    const handleToggleComplete = async (item) => {
        ...
        try {
            ...
            if (!updated) {
                throw new Error(t('shoppingList.errors.itemNotFound'));
            }
            setItems((prev) =>
                sortShoppingItems(prev.map((cur) => (cur.id === updated.id ? updated : cur)))
            );
            if (selectedItem?.id === updated.id) setSelectedItem(updated);
        } catch (toggleError) {
            console.error('Failed to toggle shopping item:', toggleError);
            setError(toggleError?.message || t('shoppingList.errors.toggle'));
        }
    };
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            setDeleting(true);
            setError('');
            await deleteShoppingItem(deleteItem.id);
            ...
            setError(deleteError?.message || t('shoppingList.errors.delete'));
        } finally {
            setDeleting(false);
        }
    };
    ...
    const filteredItems = useMemo(() => {
        ...
        return items.filter((item) => {
            if (query) {
                ...
                if (!fields.some((v) => v.includes(query))) return false;
            }

            if (status === 'pending' && Boolean(item.completed)) return false;
            if (status === 'completed' && !Boolean(item.completed)) return false;
    ...
    const statistics = useMemo(() => {
        ...
        const highPriority = items.filter(
            (i) => !i.completed && (i.priority === 'high' || i.priority === 'urgent')
        ).length;
                ...
                >
                    <div className="ui-glass-tint relative w-full max-w-md overflow-hidden rounded-2xl border border-[var(--glass-border)] shadow-[var(--shadow-xl)]">
                            ...
                            <div className="flex min-w-0 items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    <h3 className="truncate text-sm font-semibold text-[var(--text)]">
                                        {t('shoppingList.deleteModal.title')}
                                    </h3>
                                    <p className="mt-1 text-[10px] text-[var(--text-muted)]">
                                        {t('shoppingList.deleteModal.messageBefore')}
                                    </p>
                                </div>
                            </div>
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <p className="mt-2 text-[11px] leading-6 text-[var(--text-muted)]">
                                            {t('shoppingList.deleteModal.messageAfter')}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col-reverse gap-2 border-t border-[var(--border-subtle)] bg-[var(--surface-muted)] p-4 sm:flex-row sm:justify-end sm:px-5">
                            ...
                            >
                                {t('shoppingList.deleteModal.cancel')}
                            </button>
                            ...
                            >
                                {deleting ? (
                                    <>
                                        ...
                                        {t('shoppingList.deleteModal.deleting')}
                                    </>
                                ) : (
                                    <>
                                        <Trash2 size={15} />
                                        {t('shoppingList.deleteModal.delete')}
                                    </>
                                )}
                            </button>
                        </div>
