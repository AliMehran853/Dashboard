...
function SaleDetails({ sale, onClose }) {
    ...
    const amount = sale.total != null ? num(sale.total) : quantity * num(sale.unitPrice);
    const unitPrice = sale.unitPrice != null ? num(sale.unitPrice) : (quantity > 0 ? amount / quantity : 0);
    const saleId = sale.id != null ? String(sale.id) : '-';
    const dateObj = getValidDate(sale.date || sale.createdAt);

    const saleDate = dateObj
        ? (isEnglish
            ? new Intl.DateTimeFormat('en-US', { year: 'numeric', month: '2-digit', day: '2-digit' }).format(dateObj)
            : formatJalaliDate(dateObj, { monthStyle: jalaliMonthStyle, withMonthName: true }) || '-')
        : '-';

    const saleTime = dateObj
        ? new Intl.DateTimeFormat(isEnglish ? 'en-US' : 'fa-IR', { hour: '2-digit', minute: '2-digit' }).format(dateObj)
        : '-';
                ...
                <div className="ui-modal flex w-full max-w-2xl max-h-[calc(100dvh-1rem)] sm:max-h-[90vh] flex-col animate-[profileModalIn_180ms_ease-out]">
                        ...
                        <div className="flex min-w-0 flex-1 items-center gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                                <div className="flex min-w-0 items-center gap-2">
                                    ...
                                    <span dir="ltr" className="shrink-0 rounded-lg border border-[var(--border)] bg-[var(--surface-muted)] px-2 py-1 font-mono text-[10px] text-[var(--text-muted)]">
                                        #{saleId.padStart(4, '0')}
                                    </span>
                                </div>
                    ...
                    <div className="main-scrollbar flex-1 min-h-0 space-y-4 overflow-y-auto overscroll-contain p-3 sm:space-y-6 sm:p-6">
                        ...
                        <section className="relative overflow-hidden rounded-2xl border border-[var(--accent-border)] bg-[var(--accent-soft)] p-3 sm:p-5">
                            <div aria-hidden="true" className="pointer-events-none absolute -top-16 -end-16 h-40 w-40 rounded-full bg-[var(--accent-soft-heavy)] blur-3xl" />
                            <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                <div className="flex min-w-0 items-center gap-3 sm:gap-4">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <p className="truncate text-sm font-bold text-[var(--text)] sm:text-lg">{productName}</p>
                                        <div className="mt-1 flex items-center gap-2">
                                            <Tag size={12} className="shrink-0 text-[var(--text-muted)]" />
                                            <span className="truncate text-xs text-[var(--text-secondary)]">{category}</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="shrink-0 text-start">
                                    <p className="text-[10px] text-[var(--text-muted)] sm:text-[11px]">{t('sales.details.totalAmount')}</p>
                                    <p dir="ltr" className="number-font mt-1 text-lg font-bold text-[var(--accent-500)] sm:text-2xl">
                                        {fmt(amount)}
                        ...
                        <section>
                            ...
                            <div className={`flex flex-col gap-4 rounded-xl border p-3 sm:flex-row sm:items-center sm:justify-between sm:p-4 ${isCash ? 'border-cyan-500/15 bg-cyan-500/5' : 'border-amber-500/15 bg-amber-500/5'}`}>
                                <div className="flex min-w-0 items-center gap-3">
                                    <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${isCash ? 'bg-cyan-500/10' : 'bg-amber-500/10'}`}>
                                        {isCash ? <Banknote size={19} className="text-cyan-500" /> : <CreditCard size={19} className="text-amber-500" />}
                                    </div>
                                    <div className="min-w-0 flex-1">
                                        <p className="truncate text-sm font-medium text-[var(--text)]">
                                            {isCash ? t('sales.details.paymentStatus.cashTitle') : t('sales.details.paymentStatus.creditTitle')}
                                        </p>
                                        <p className="mt-1 text-[11px] leading-5 text-[var(--text-secondary)]">
                                            {isCash ? t('sales.details.paymentStatus.cashDescription') : t('sales.details.paymentStatus.creditDescription')}
                                        </p>
                                    </div>
                                </div>
                                <div className="flex shrink-0 items-center gap-1.5 self-start sm:self-auto">
                                    {isCash ? <CheckCircle2 size={16} className="text-cyan-500" /> : <Clock3 size={16} className="text-amber-500" />}
                                    <span className={`text-xs font-medium ${isCash ? 'text-cyan-500' : 'text-amber-500'}`}>
                                        {isCash ? t('sales.details.paymentStatus.settled') : t('sales.details.paymentStatus.unpaid')}
                                    </span>
                                </div>
                            </div>
                        </section>
                        ...
                        <section>
                            ...
                            <div className="grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--border)] sm:grid-cols-4">
                                <InfoItem icon={Hash} label={t('sales.details.saleInformation.quantity')} value={`${fmt(quantity)} ${t('sales.details.units')}`} />
                                <InfoItem icon={DollarSign} label={t('sales.details.saleInformation.unitPrice')} value={`${fmt(unitPrice)} ${t('common.currency')}`} />
                                <InfoItem icon={CalendarDays} label={t('sales.details.saleInformation.date')} value={saleDate} />
                                <InfoItem icon={Clock3} label={t('sales.details.saleInformation.time')} value={saleTime} />
                            </div>
                        </section>
                        ...
                        <section>
                            ...
                            <div className="rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] p-4">
                                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                    <CustomerItem icon={User} label={t('sales.details.customer.name')} value={customerName} />
                                    <CustomerItem icon={Phone} label={t('sales.details.customer.phone')} value={customerPhone || '-'} dir="ltr" />
                                </div>
                            </div>
                        </section>
                    ...
                    <div className="ui-modal-footer flex shrink-0 flex-col-reverse gap-3 px-3 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6 sm:py-4">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[11px] text-[var(--text-muted)]">{t('sales.details.footer.recorded')}</span>
                        </div>
                        <button type="button" onClick={() => onClose?.()} className="ui-button-secondary h-10 w-full rounded-xl px-5 sm:w-auto">
    ...
    return (
        <div className="min-w-0 bg-[var(--surface)] p-3 sm:p-4">
            <div className="flex items-center gap-2">
                ...
                <span className="min-w-0 truncate text-[10px] text-[var(--text-muted)] sm:text-[11px]">{label}</span>
            </div>
            <p className="mt-2 truncate text-xs font-semibold text-[var(--text)] sm:text-sm">{value}</p>
        </div>
    );
}
    ...
    return (
        <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
                ...
                <p dir={dir} className="mt-1 truncate text-sm text-[var(--text-secondary)]">{value}</p>
            </div>
        </div>
    );
}
