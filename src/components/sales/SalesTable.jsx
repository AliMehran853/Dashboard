...
import { Eye, ShoppingBag, Loader2, Receipt } from 'lucide-react';
...
function SalesTable({ filters = {}, onViewSale }) {
    ...
    const filteredSales = sales.filter((sale) => {
        ...
        if (search) {
            const text = [sale.productName, sale.category, sale.customerName, sale.customerPhone, sale.note]
                .filter(Boolean).join(' ').toLowerCase();
            if (!text.includes(search)) return false;
        }
        if (paymentType !== 'all' && sale.paymentType !== paymentType) return false;
    ...
    const formatDate = (value) => {
        ...
        return formatJalaliDate(value, { monthStyle: jalaliMonthStyle, withMonthName: true }) || '-';
    };
    ...
    const formatTime = (value) => {
        ...
        const d = new Date(value);
        if (Number.isNaN(d.getTime())) return '';
        return new Intl.DateTimeFormat(isEnglish ? 'en-US' : 'fa-IR', { hour: '2-digit', minute: '2-digit' }).format(d);
    };
        ...
        return (
            <section dir={isEnglish ? 'ltr' : 'rtl'} className="ui-card flex min-h-64 items-center justify-center p-6">
                <div className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-sm text-[var(--text-muted)]">{t('sales.table.loading')}</span>
                </div>
            </section>
        );
    }
    ...
    if (filteredSales.length === 0) {
        return (
            <section dir={isEnglish ? 'ltr' : 'rtl'} className="ui-card flex min-h-64 flex-col items-center justify-center px-6 py-10 text-center">
                ...
                <h3 className="text-sm font-semibold text-[var(--text)]">{t('sales.table.empty.title')}</h3>
                <p className="mt-2 max-w-md text-xs leading-5 text-[var(--text-muted)]">{t('sales.table.empty.description')}</p>
            </section>
        );
    }

    return (
        <section dir={isEnglish ? 'ltr' : 'rtl'} className="ui-card w-full min-w-0 overflow-hidden p-0">
            {/* header */}
            <div className="flex items-center justify-between gap-4 border-b border-[var(--border)] px-4 py-4 sm:px-5 sm:py-5 lg:px-6">
                <div className="flex min-w-0 items-center gap-3">
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[var(--accent-border)] bg-[var(--accent-soft)] shadow-[var(--shadow-xs)]">
                ...
                <table className="w-full min-w-[920px] border-collapse">
                    <thead>
                        <tr className="border-b border-[var(--border)] bg-[var(--surface-muted)] text-[11px] text-[var(--text-muted)]">
                            ...
                            <Th>{t('sales.table.columns.payment')}</Th>
                    ...
                    <tbody>
                        {filteredSales.map((sale) => {
                            ...
                            return (
                                <tr key={sale.id} className="group border-b border-[var(--border)]/70 transition-colors duration-200 last:border-b-0 hover:bg-[var(--surface-hover)]">
                                    {/* product */}
                                    <td className="px-5 py-4">
                                        <div className="flex min-w-0 items-center gap-3">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                {sale.customerName && (
                                                    <p className="mt-1 truncate text-[10px] text-[var(--text-muted)]">{sale.customerName}</p>
                                                )}
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    {/* payment */}
                                    <td className="px-5 py-4">
                                        <span className={`inline-flex items-center rounded-lg border px-2.5 py-1 text-[11px] font-medium ${isCredit ? 'border-amber-500/15 bg-amber-500/10 text-amber-500' : 'border-cyan-500/15 bg-cyan-500/10 text-cyan-500'}`}>
                                            {isCredit ? t('sales.paymentTypes.credit') : t('sales.paymentTypes.cash')}
                                        </span>
                                    </td>
            ...
            <div className="divide-y divide-[var(--border)] bg-[var(--surface)] md:hidden">
                {filteredSales.map((sale) => {
                    const isCredit = sale.paymentType === 'credit';
                    return (
                        <article key={sale.id} className="relative p-4 transition-colors duration-200 hover:bg-[var(--surface-hover)]">
                            ...
                            <div className="mt-4 rounded-xl border border-[var(--accent-border)] bg-[var(--accent-soft)] px-3 py-3">
                                <div className="flex items-center justify-between gap-3">
                                    <div className="min-w-0">
                                        <p className="text-[10px] text-[var(--text-muted)]">
                                            {t('sales.table.columns.amount')}
                                        </p>
                                        <p dir="ltr" className="number-font mt-1 truncate text-lg font-bold text-[var(--accent-500)]">
                                            {fmtMoney(getSaleTotal(sale))}
                                    ...
                                    <div className="shrink-0 text-end">
                                        <span className={`inline-flex rounded-lg border px-2.5 py-1.5 text-[10px] font-medium ${isCredit ? 'border-amber-500/15 bg-amber-500/10 text-amber-500' : 'border-cyan-500/15 bg-cyan-500/10 text-cyan-500'}`}>
                                            {isCredit ? t('sales.paymentTypes.credit') : t('sales.paymentTypes.cash')}
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div className="mt-3 rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] p-3">
                                <div className="grid grid-cols-2 gap-3">
                                    <InfoItem label={t('sales.table.columns.quantity')} value={fmtNumber(sale.quantity)} />
                                    <InfoItem align="end" label={t('sales.form.fields.unitPrice')} value={`${fmtMoney(sale.unitPrice)} ${t('common.currency')}`} />
                                </div>
                                <div className="my-3 h-px bg-[var(--border)]" />
                                <div className="grid grid-cols-2 gap-3">
                                    <InfoItem label={t('sales.table.columns.date')} value={formatDate(sale.date || sale.createdAt)} />
                                    <InfoItem align="end" label={t('sales.table.columns.date')} value={formatTime(sale.date || sale.createdAt)} />
                                </div>
                                {sale.customerName && (
                                    <>
                                        <div className="my-3 h-px bg-[var(--border)]" />
                                        <InfoItem label={t('sales.details.customer.name')} value={sale.customerName} />
                                    </>
                                )}
                            </div>
                        </article>
...
function Th({ children }) {
    return (
        <th className="whitespace-nowrap px-5 py-4 text-start font-medium">{children}</th>
    );
}
...
function InfoItem({ label, value, align = 'start' }) {
    return (
        <div className={`min-w-0 ${align === 'end' ? 'text-end' : 'text-start'}`}>
            ...
            <p className="truncate text-xs font-medium text-[var(--text-secondary)]">{value}</p>
        </div>
    );
}
