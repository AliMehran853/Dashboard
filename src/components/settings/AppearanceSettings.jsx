...
// =========================================================
// Accent palette factory
// =========================================================

const getAccentColors = (t) => [
    { name: 'emerald', label: t('settings.appearance.colors.emerald.name'), description: t('settings.appearance.colors.emerald.description'),
      colorClass: 'bg-emerald-500', ringClass: 'ring-emerald-500/30', textClass: 'text-emerald-500', borderClass: 'border-emerald-500/40', bgClass: 'bg-emerald-500/10' },
    { name: 'blue',    label: t('settings.appearance.colors.blue.name'),    description: t('settings.appearance.colors.blue.description'),
      colorClass: 'bg-blue-500',    ringClass: 'ring-blue-500/30',    textClass: 'text-blue-500',    borderClass: 'border-blue-500/40',    bgClass: 'bg-blue-500/10' },
    { name: 'violet',  label: t('settings.appearance.colors.violet.name'),  description: t('settings.appearance.colors.violet.description'),
      colorClass: 'bg-violet-500',  ringClass: 'ring-violet-500/30',  textClass: 'text-violet-500',  borderClass: 'border-violet-500/40',  bgClass: 'bg-violet-500/10' },
    { name: 'amber',   label: t('settings.appearance.colors.amber.name'),   description: t('settings.appearance.colors.amber.description'),
      colorClass: 'bg-amber-500',   ringClass: 'ring-amber-500/30',   textClass: 'text-amber-500',   borderClass: 'border-amber-500/40',   bgClass: 'bg-amber-500/10' },
    { name: 'cyan',    label: t('settings.appearance.colors.cyan.name'),    description: t('settings.appearance.colors.cyan.description'),
      colorClass: 'bg-cyan-500',    ringClass: 'ring-cyan-500/30',    textClass: 'text-cyan-500',    borderClass: 'border-cyan-500/40',    bgClass: 'bg-cyan-500/10' },
    { name: 'rose',    label: t('settings.appearance.colors.rose.name'),    description: t('settings.appearance.colors.rose.description'),
      colorClass: 'bg-rose-500',    ringClass: 'ring-rose-500/30',    textClass: 'text-rose-500',    borderClass: 'border-rose-500/40',    bgClass: 'bg-rose-500/10' },
];
...
function AppearanceSettings() {
    ...
    const colors = getAccentColors(t);
                            ...
                            return (
                                <button
                                    ...
                                    className={`group relative flex w-full items-center gap-3 rounded-xl border p-3 text-start transition-all duration-200 ${
                                        isActive
                                            ? `${color.borderClass} ${color.bgClass} shadow-sm`
                                            : 'border-[var(--border)] bg-[var(--surface-muted)] hover:border-[var(--border-strong)] hover:bg-[var(--surface)]'
                                    }`}
                                >
                                    <span
                                        className={`h-9 w-9 shrink-0 rounded-xl ${color.colorClass} ring-4 transition-all duration-200 ${
                                            isActive ? color.ringClass : 'ring-transparent'
                                        }`}
                                    />
                                    <span className="min-w-0 flex-1">
                                        <span className={`block truncate text-xs font-semibold ${
                                            isActive ? color.textClass : 'text-[var(--text-primary)]'
                                        }`}>
                                            {color.label}
                                        </span>
                                    ...
                                    {isActive && (
                                        <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full ${color.colorClass}`}>
