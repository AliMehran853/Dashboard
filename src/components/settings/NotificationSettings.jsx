...
import {
    Bell, ShoppingCart, AlertTriangle, CreditCard, CheckCircle2,
} from 'lucide-react';
import { useTranslation } from 'react-i18next';

const NOTIFICATION_STORAGE_KEY = 'taqwa-notifications';
...
const getSavedNotifications = () => {
    try {
        ...
        return {
            ...
            newSale: parsed.newSale !== false,
            credit: parsed.credit !== false,
            successfulActions: parsed.successfulActions !== false,
        };
    } catch (error) {
...
function NotificationSettings() {
    ...
    const items = [
        {
            ...
            description: t('settings.notifications.items.lowStock.description'),
            icon: AlertTriangle,
        ...
        },
        {
            ...
            description: t('settings.notifications.items.newSale.description'),
            icon: ShoppingCart,
        ...
        },
        {
            ...
            description: t('settings.notifications.items.credit.description'),
            icon: CreditCard,
            iconClass: 'bg-rose-500/10 text-rose-500 dark:text-rose-400',
        },
        {
            id: 'successfulActions',
            title: t('settings.notifications.items.successfulActions.title'),
            description: t('settings.notifications.items.successfulActions.description'),
            icon: CheckCircle2,
    ...
    const handleToggle = (id) => {
        setNotifications((current) => {
            const next = { ...current, [id]: !Boolean(current[id]) };
            try {
        ...
        <section dir={i18n.dir()} className="ui-card overflow-hidden p-0">
            ...
            <div className="flex items-center gap-3 border-b border-[var(--border)] px-4 py-4 sm:px-6 sm:py-5">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-cyan-500/10 bg-cyan-500/10">
                    <Bell size={19} className="text-cyan-500 dark:text-cyan-400" />
                </div>
            ...
            <div className="space-y-3 p-4 sm:p-6">
                {items.map((item) => {
                    ...
                    const enabled = Boolean(notifications[item.id]);
                    const thumbPosition = enabled
                    ...
                    return (
                        ...
                        >
                            <div className="flex min-w-0 flex-1 items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    <p title={item.title} className="truncate text-sm font-semibold text-[var(--text-primary)]">
                                        {item.title}
                                    </p>
