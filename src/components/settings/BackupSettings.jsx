...
import {
    DatabaseBackup, Download, Upload, ShieldCheck, FileJson,
    Loader2, CheckCircle2, AlertTriangle, HardDrive,
} from 'lucide-react';
...
const BACKUP_TYPE = 'taqwa-full-backup';
...
const downloadJsonFile = (json) => {
    ...
    link.download = `taqwa-shop-backup-${date}.json`;
...
const validateBackup = (backup) => {
    ...
    if (backup.app !== BACKUP_APP) return false;
    if (backup.type !== BACKUP_TYPE) return false;
    if (!backup.database || typeof backup.database !== 'object') return false;
    if (!backup.database.tables || typeof backup.database.tables !== 'object') return false;
    if (!backup.localStorage || typeof backup.localStorage !== 'object') return false;
    return true;
};
...
const restoreDatabase = async (tables) => {
    ...
    for (const table of currentTables) {
        ...
        if (!Array.isArray(rows)) throw new Error(`Invalid table data: ${table.name}`);
        tablesToRestore.push({ table: currentTableMap.get(table.name), rows });
    }

    await db.transaction('rw', ...currentTables, async () => {
        for (const table of currentTables) await table.clear();
...
function BackupSettings() {
    ...
    const handleBackup = async () => {
        try {
            ...
            const backup = {
                ...
                database: { version: db.verno, tables: database },
                localStorage: getLocalStorageData(),
            };

            downloadJsonFile(JSON.stringify(backup, null, 4));
            setMessage(t('settings.backup.messages.exportSuccess'));
        } catch (backupError) {
            console.error('Backup failed:', backupError);
            setError(t('settings.backup.errors.export'));
        } finally {
            setExporting(false);
        }
    };
    ...
    const handleImport = async (event) => {
        ...
        try {
            ...
            if (!validateBackup(backup)) throw new Error('Invalid Taqwa backup file');

            const confirmed = window.confirm(t('settings.backup.import.confirm'));
            if (!confirmed) return;

            await restoreDatabase(backup.database.tables);
            restoreLocalStorage(backup.localStorage);

            setMessage(t('settings.backup.messages.importSuccess'));

            ['database-updated', 'products-updated', 'sales-updated',
             'categories-updated', 'customers-updated', 'credit-updated',
             'shopping-list-updated'].forEach((name) => {
            ...
            setError(t('settings.backup.errors.import'));
        } finally {
            setImporting(false);
            event.target.value = '';
        }
        ...
        <section dir={i18n.dir()} className="ui-card overflow-hidden p-0">
            ...
            <div className="flex items-center gap-3 border-b border-[var(--border)] px-4 py-4 sm:px-6 sm:py-5">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-cyan-500/10 bg-cyan-500/10">
                    <DatabaseBackup size={19} className="text-cyan-500 dark:text-cyan-400" />
                </div>
            ...
            <div className="space-y-5 p-4 sm:p-6">
                ...
                <div className="flex items-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-muted)] px-4 py-3">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-slate-500/10">
                        <FileJson size={18} className="text-[var(--text-muted)]" />
                    </div>
                    <div className="min-w-0">
                ...
                {/* Messages */}
                {message && (
                    <StatusBanner tone="success" icon={CheckCircle2} message={message} />
                )}
                {error && (
...
function OperationCard({
    ...
    buttonLabel, buttonIcon: ButtonIcon, buttonClass,
    spinning, disabled, onClick, children,
}) {
    const toneMap = {
        emerald: {
    ...
    return (
        <div className={`group relative overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface-muted)] p-4 transition-all duration-300 sm:p-5 ${styles.border}`}>
            <div className={`pointer-events-none absolute -end-12 -top-12 h-32 w-32 rounded-full blur-3xl opacity-0 transition-opacity duration-500 group-hover:opacity-100 ${styles.glow}`} />

            <div className="relative z-10 flex flex-col gap-4">
                <div className="flex items-start gap-3">
                    <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border ${styles.iconWrap}`}>
                ...
                >
                    <ButtonIcon size={16} className={spinning ? 'animate-spin' : ''} />
                    {buttonLabel}
                </button>
...
function StatusBanner({ tone, icon: Icon, message }) {
    ...
    return (
        <div role={tone === 'success' ? 'status' : 'alert'} className={`flex items-start gap-3 rounded-xl border px-4 py-3 text-sm leading-5 ${classes}`}>
