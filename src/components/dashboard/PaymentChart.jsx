...
function PaymentChart({ sales = [], loading = false }) {
    ...
    const tokens = useMemo(() => ({
        ...
        warning: getCssVar('--warning', '#d97706'),
    ...
    const options = useMemo(() => ({
        ...
        colors: [tokens.accent, tokens.warning],
        ...
        responsive: [
            { breakpoint: 768, options: { plotOptions: { pie: { donut: { size: '70%' } } } } },
            { breakpoint: 480, options: { chart: { height: 220 }, plotOptions: { pie: { donut: { size: '66%' } } } } },
        ],
            ...
            <div className="flex items-center gap-2.5">
                <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border ${
                    tone === 'accent'
                        ...
                        : 'border-amber-500/15 bg-amber-500/[0.07] text-amber-500 dark:text-amber-400'
                }`}>
    ...
    return (
        <section dir={i18n.dir()} className="ui-card h-full min-w-0 overflow-hidden p-4 sm:p-5">
            ...
            <div className="mt-3 grid grid-cols-1 min-[380px]:grid-cols-2 gap-2 sm:gap-3">
                ...
                <SummaryCard icon={CreditCard} title={t('dashboard.paymentChart.credit')} value={creditSales} tone="warning" />
            </div>
        </section>
    );
}
